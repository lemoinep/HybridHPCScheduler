...
// Version 1.2 05/06/2026 — CPU = logical processors (PU), --real mode
// For Windows
...
struct SimConfig {
    ...
    int cpu_speed = 120;
    int cpu_threads_per_socket = -1; // unused in PU mode but kept for compatibility
    ...
    bool detect_cpu_real = true;   // CPUs via hwloc (PUs)
    bool detect_gpu_real = false;  // disabled on Windows for now
    bool add_simulated_gpu = true; // simulated GPUs

    bool real_only = false;        // new: real hardware only mode
};
...
int main(int argc, char** argv) {
    ...
    add_arg_bool(argc, argv, "--no-real-cpu", cfg.detect_cpu_real);   // not really used now but kept
    add_arg_bool(argc, argv, "--no-real-gpu", cfg.detect_gpu_real);   // unused on Windows
    ...
    // If real-only mode, disable all simulated accelerators
    if (cfg.real_only) {
        cfg.add_simulated_gpu = false;
        cfg.npu_count = 0;
        cfg.tpu_count = 0;
        cfg.dpu_count = 0;
        // On Windows, we currently have no real GPU detection, so cfg.detect_gpu_real stays false.
    }
    ...
    // PUs = logical processors (CPU 0..CPU N in Windows Resource Monitor)
    int pus  = nbobjs(topo, HWLOC_OBJ_PU);
    int cpus = (std::max)(1, pus);

    // Each CPU device = 1 logical processor → 1 thread
    ...
    if (cfg.detect_cpu_real) {
        for (int i = 0; i < cpus; ++i) {
            DeviceInfo d;
            d.name       = cpu_name(i);                 // CPU0..CPUn
            d.kind       = "cpu";
            d.speed      = cfg.cpu_speed;
            d.threads    = cfg.cpu_threads_per_socket;  // 1
...
#if defined(_WIN32)
    cfg.detect_gpu_real = false;
#endif

    // Simulated GPUs (disabled in --real mode)
    ...
    // Simulated NPUs / TPUs / DPUs (disabled in --real mode by setting counts to 0)
    ...
    auto add_dep = [&](const std::string& src, const std::string& dst, int bytes) {
        if (src.empty() || dst.empty()) return; // skip invalid in real-only mode
        YAML::Node d;
        d["src"]   = src;
        d["dst"]   = dst;
        d["bytes"] = bytes;
    ...
    emitter << root;                  // serialize YAML tree [web:156][web:141][web:209]
    fout << emitter.c_str();
